---
title: "About"
date: 2020-01-05T22:28:38+01:00
draft: false
category: aboutme
tags: markdown, cv
---

----

                                        Sandeep Lamba - DevOps Jedi

----

>  The DevOps Jedi who can save your day

>  Self-motivated individual with 10+ years of IT experience

>  Proficient in architecting and automating the infrastructure with AWS, Kubernetes and Terraform

>  Experience in building scalable infrastructure, build and deployment

>  Proficient in setting up the frameworks for

>    - Public/On-Prem Clouds
>    - Kubernetes Clusters Public/On-Prem Clouds
>    - Logging
>    - Monitoring
>    - Alerting
>    - Backup/Restore Databases
>    - Disaster Recovery Management

----

                                              Education

----

**Army Institute of Technology, Pune(India)**  

> B.E. in Computer Science  
> *A+ in University*  
> *July 2007*  

----

                                              Skills / Tools

----

**Current Tech Stack** 

_Nudging around with below technologies and languages_

> <p align="left">
>   <img src="/images/tech-stack.png" width="585"> </image>
> </p>

_Languages_

> <p align="left">
>   <img src="/images/languages.png" width="285"> </image>
> </p>

---- 

                                              Experience

----

**Senior DevOps Engineer, [Grand-Parade][6], Krakow(Poland)** 

> *March 2018 &ndash; Present*

> - Member of Platform Team
> - Moving On-Prem applications to AWS
> - Nudging around with below technologies
> <p align="left">
>   <img src="/images/tech-stack.png" width="585"> </image>
> </p>

**Devops, Senior System Administrator, [Tabella][5], Prague(Czech Republic)**  

> *October 2016 &ndash; January 2018*  

> - 𝗠𝗮𝗶𝗹-𝗦𝗲𝗿𝘃𝗲𝗿-𝗔𝗱𝗺𝗶𝗻𝗶𝘀𝘁𝗿𝗮𝘁𝗶𝗼𝗻(𝗭𝗶𝗺𝗯𝗿𝗮)
> - 𝗘𝗦𝗫𝗶-𝗔𝗱𝗺𝗶𝗻𝗶𝘀𝘁𝗿𝗮𝘁𝗶𝗼𝗻
> - 𝗦𝗲𝗿𝘃𝗲𝗿-𝗩𝗣𝗦-𝗔𝗱𝗺𝗶𝗻𝗶𝘀𝘁𝗿𝗮𝘁𝗶𝗼𝗻
> - 𝗗𝗡𝗦-𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁
> - 𝗖𝗗𝗡-𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁
> - 𝗣𝗲𝗿𝗳𝗼𝗿𝗺𝗮𝗻𝗰𝗲-𝗧𝘂𝗻𝗶𝗻𝗴-𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁
> - 𝗟𝗼𝗴𝘀-𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁
> - 𝗔𝗹𝗲𝗿𝘁-𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁
> - 𝗖𝗜-𝗖𝗗-𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁
> - 𝗖𝗼𝗻𝘁𝗲𝗻𝘁-𝗠𝗮𝗻𝗮𝗴𝗲𝗺𝗲𝗻𝘁
> - 𝗧𝗼𝗼𝗹𝘀-𝗦𝘁𝗮𝗰𝗸
> <p align="left">
>   <img src="/images/tech-stack-tabella.png" width="585"> </image>
> </p>

**Systems and Integration Enginner, [Gemalto][4], Prague(Czech Republic)**  
> *December 2015 &ndash; November 2016*  

> - Integration Activities for developers and validators
> - Create VM's , Application Deployment(WebSphere, Tomcat JBOSS)
> - DB installations(Oracle, DB2, MySQL, MS-SQL)
> - Creating Master images for relevant RHEL OS versions
> - RHEL7.2, vSphere Administration, Python, Shell, IBM WebSphere, IBM IHS, IBM Performance 
> - Do the automation and Orchestration of integration tasks
> - Tools stack Ansible, Docker, Grinder, RunDeck, Jenkins, Graphana, Graphite, CollectD, StatsD
> - WebSphere Clustering, Apache Proxy, IBM HTTP Server(IHS) as Reverse Proxy
> - Automation Scripts in Shell, Python, Simple Java programs

**Systems Enginner, [Gemalto][4], Singapore**  
> *January 2012 &ndash; December 2015*  

> - Integration in Data Center Singapore 
> - Includes installation of Gemalto products, firewall configuration
> - To Apply security rules and maintaining infrastructure
> - Lead Team of 4 in Singapore
> - Worked with World wide Mobile Network Operators 
> - And Service Providers from ASIA, JAPAN, US and EUROPE on NFC implementation on TSM products
> - Tools Stack : UNIX, Python, Perl Scripting, Weblogic, JBOSS, VCS, Network Security
> - Web Service Security, TLS Mutual Authentication, Apache Server, NGM Monitoring, Oracle PL/SQL

**IT Support Analyst, [Tech-Mahindra][3], Milton Keynes(UK)**  
> *September 2008 &ndash; July 2011*  

> - Responsible to provide Second and Third line support for British Telecom applications
> - Providing data correction scripts for new code
> - Proactive Maintenance/Vigilance of the linux system applications and processes
> - Provide software Code Changes, bug fixes and general maintenance of the product
> - Liaising with Testing Team to resolve Code Level Changes and testing and pusing to Live

**Testing Analyst, [Tech-Mahindra][3], Pune(India)**  
> *July 2007 &ndash; August 2008*  

> - Automation of the Application with tool WinRunner
> - Writing scripts to do Regression-Testing of applications using WinRunner

---- 

                                        Licenses & Certifications
**AWS Certified Solutions Architect - Associate (SAA)**  
> *Issuing Organization - Amazon Web Services (AWS)*  

> - Issue Date - March, 2019
> - Expiration Date - March, 2022
> - Credentials ID - LP3TBB6KLMRQ1CGR
> - [Credential URL][2]
> <p align="left">
> <img src="/images/AWS-SAA-Badge.png" width="185"> </image>
> </p>

**Certified Kubernetes Administrator**  
> *Issuing Organization - CNCF [Cloud Native Computing Foundation]*  

> - Issue Date - August, 2018
> - Expiration Date - August, 2021
> - Credentials ID - CKA-1800-0677-0100
> - [Credential URL][1]
> <p align="left">
> <img src="/images/logo_cka.png" width="185"> </image>
> </p>

| Issuing Organization | Issue Date | Expiration Date | Credentials ID | Badge |
| --- | --- | --- | --- | --- | 
| `Amazon Web Services (AWS)`| March, 2019 | March, 2022 | LP3TBB6KLMRQ1CGR | [<p align="left"> <img src="/images/AWS-SAA-Badge.png" width="185"> </image> </p>](https://www.certmetrics.com/amazon/public/badge.aspx?i=1&t=c&d=2019-03-14&ci=AWS00379394) | 
| `CNCF [Cloud Native Computing Foundation]`| August, 2018 | August, 2021 | CKA-1800-0677-0100 | [<p align="left"> <img src="/images/logo_cka.png" width="185"> </image> </p>](https://portal.linuxfoundation.org/certification/verify-certifications) | 
| ` `|  | ` ` | | | 

---

[1]: https://portal.linuxfoundation.org/certification/verify-certifications
[2]: https://www.certmetrics.com/amazon/public/badge.aspx?i=1&t=c&d=2019-03-14&ci=AWS00379394
[3]: https://www.techmahindra.com/cwce.html
[4]: https://www.gemalto.com
[5]: http://tabella.cz
[6]: https://grandparade.co.uk